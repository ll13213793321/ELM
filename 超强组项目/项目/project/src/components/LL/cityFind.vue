<template>
  <div id="app1">
    ...
    <div>
      <ul>
        <li
          ...
          :key="i"
          id="buttom_search"
          @click="localS(v.name,v.address,v.geohash)"
        >
          <router-link :to="'/home?geohash='+v.geohash">
            <p
              @click="setMessage(v.geohash)"
              v-html="v.name"
              class="search"
            ></p>
            <p
    ...
    >
      ...
      <ul>
        <li
          v-for="(v,i) in localName"
          :key="i"
          id="buttom_search"
        >
          <router-link :to="'/home?geohash='+localgeohash[i]">
            <p
              @click="setMessage(localgeohash[i])"
              v-html="v"
              class="search"
            ></p>
            <p
              v-html="localAddress[i]"
              class="search search_p"
            ></p>
          </router-link>
        </li>
...
export default {
  ...
  methods: {
    ...
    setMessage(value) {
      this.$store.commit("getMessage1", value);
      console.log(this.$store.state.Allgeohash);
    }
  },
  created() {
