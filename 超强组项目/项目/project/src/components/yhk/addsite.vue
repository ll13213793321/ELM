  ...
  <div id="addsite">
    ...
    <div id="content">
      <input
        ...
        v-model="name"
      />
      <p class="error" v-if="show2">请填写你的姓名</p>
      ...
      <input
        ...
        v-model="phone"
      />
      <p class="error" v-if="show1">手机号不能为空</p>
      <input class="inp" type="text" placeholder="备用联系电话（选填）" v-model="phone_bk" />
    </div>
    <button class="btn" @click="go()">新增地址</button>
  </div>
</template>
<script>
export default {
  ...
  methods: {
    ...
    change2() {
      if (!/[\u4E00-\u9FA5\\s]{2,4}$/.test(this.address_detail)) {
        this.bor2 = "1px solid red";
        this.show = true;
      } else {
        this.bor = 0;
        this.show = false;
    ...
    go() {
      ...
      this.$http({
        ...
        data: {
          ...
          geohash: this.$store.state.Allgeohash,
          name: this.name,
          phone: this.phone,
          tag: "a",
          sex: 1,
